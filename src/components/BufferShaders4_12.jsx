 ...
 * It now accepts `gapFactor`, `spacingMultiplier`, and `position` to control the layout and placement.
 */
const BufferScene = ({ gapFactor, spacingMultiplier, position = [0, 0, 0] }) => {
    const mesh = useRef();
...
const CameraAnimator = () => {
    ...
    useFrame((state) => {
        ...
        // Cinematic orbit: slow rotation around the Y-axis with some variation in distance and height
        const radius = 30 + Math.sin(time * 0.05) * 5;
        camera.position.x = Math.sin(time * 0.05) * radius;
        camera.position.z = Math.cos(time * 0.05) * radius;
        camera.position.y = 5 + Math.sin(time * 0.1) * 5;
        camera.lookAt(0, 0, 0);
    });
    return null;
};
...
const BufferShaders4_12 = () => {
    ...
    // Define positions for multiple galaxies
...
export default BufferShaders4_12;